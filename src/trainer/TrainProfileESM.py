...
class TrainProfileESM(TrainModel):
    ...
    def __init__(self, nametrain, device, path_dir=".", ckpt_dir='checkpoints'):
        super().__init__(nametrain, device, path_dir, ckpt_dir)

    
    def train(self, train_loader, criterion, epoch):
        ...
        with tqdm(range(num_batches)) as pbar:
            for idx_batch, (data_map, targets) in zip(pbar, train_loader):
                ...
                class_targets = targets[:,0].to(self.device)
    ...
    def validation(self, val_loader, criterion, epoch):
        ...
        with torch.no_grad():
            with tqdm(range(num_batches)) as pbar:
                for idx_batch, (data_map, targets) in zip(pbar, val_loader):
                    ...
                    loss_val += loss.item() * len(targets)
